...
function handleFormSubmit(e) {
  ...
  btn.disabled = true;
  btn.innerHTML = '<i class="ti ti-loader-2" aria-hidden="true" style="animation:spin 1s linear infinite"></i> Sending…';

  // Send the actual form data to Web3Forms
  ...
  .then(async (response) => {
    if (response.status === 200) {
      ...
      // Re-enable after 8 s in case user wants to send another
      setTimeout(() => {
        btn.style.display = '';
        btn.disabled      = false;
...
document.querySelectorAll('.apply-form input, .apply-form select, .apply-form textarea').forEach(el => {
